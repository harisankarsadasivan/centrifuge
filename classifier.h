...
#include "util.h"
#include <iostream>
//HARI how many target reads to collect
#define TARGET_READ 100000

extern int TARGET_READS_ACHIEVED(0);
...
public:
	...
	if ( reported == false ) 
		reportUnclassified( sink ) ;
		std::cout<<"Read no: "<<++z<<"---------Unclassified READ--------\n";
	}
        
	//HARI
	else
	{
	    std::cout<<"Read no: "<<++z<<"---------Classified READ--------\n";
	    count_target();
    ...
    void count_target()
    {
	    ...
	    target_read++;
	    std::cout<< target_read<<" target reads identified\n";
	    if (target_read == TARGET_READ){
		    std::cout<< "----------REQUIRED NUMBER OF TARGET READS OBTAINED......PROGRAM TERMINATES NOW!!!!!!!!------\n";
		    TARGET_READS_ACHIEVED=1;
	    }
    }
