	...
	const string& outfile)
{
	...
	fileCnt += sra_accs ;
#endif
	static ReadfromFIFO fifo[1];
	TARGET_READS_ACHIEVED=0;
	...
	struct dirent *ent;
	int read_no=1;
	std::string fname="/tmp/ONT";
	std::string fn = fname + "/1.fastq";
	...
	while(1 && !TARGET_READS_ACHIEVED)
	
	//for ( int fileIdx = 0 ; fileIdx < fileCnt ; ++fileIdx )
	{
		std::cout<<"Iteration:" << read_no<<"\n";
		...
		{
               //      if(fifo[0].ReadFIFOValid()=='V'){

		std::cout<<"entered 2nd if\n";
			// single end read
			//HARI

	        //fifo[0].WritetoInMemFile();

			queries[fileIdx - mates1.size()]=fn;
			query.push_back( queries[ fileIdx - mates1.size() ] ) ;
			std::cout<<"READ read no: " <<read_no<<"\n";
		//	}//
		/*	else{
		//		std::cout<<"Unable to find valid FIFO entry....waiting.....\n";
			}*/
		...
		read_no++;
		fn=fname+"/"+std::to_string(read_no)+".fastq";
		while(access(fn.c_str(),R_OK)!=0);
		{
			std::cout<<"waiting for access to file";
		};
		std::cout<< "Access of"<<fn <<"is possible" << access(fn.c_str(),R_OK)<<"\n";
	} // end for-loop of fileCnt
	std::cout<<"::::::::::::SENDING RUN_UNTIL STOP SIGNAL TO DEVICE::::::::::::::::::::::::::::::::";
	system("/opt/ONT/minknow_api/examples/stop_run.sh");
	...
	if ( !separator )
	{
		if (!reportFile.empty()) {
			...
			reportOfb << "abundance" << '\t' << "Total_BP_Len" << '\t' << "No of individuals"; 
			if(false) {
				reportOfb << '\t' << "abundance_normalized_by_genome_size";
			}
			reportOfb << endl;
			for(map<uint64_t, ReadCounts>::const_iterator it = spm.species_counts.begin(); it != spm.species_counts.end(); ++it) {
				...
				reportOfb <<'\t' <<it->second.total_bp << '\t' <<(float)it->second.total_bp/(float)genome_size;
				reportOfb << endl;
